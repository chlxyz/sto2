    ...
    @foreach ($products as $product)
        <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
            ...
            <div>
                ...
                <p class="text-sm">Quantity: {{ $product->product_quantity }}</p>
            </div>
            <div>
                <a href="{{ route('edit.productform', ['id' => $product->id]) }}" class="text-blue-500">Edit</a>
                <form action="{{ route('admin.deleteproduct', ['id' => $product->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this product?')">
                    @csrf
                    @method('DELETE')
